...
async def test_upload_avatar_success(client):
    token = await register_and_login(client, unique_email(), "freelancer")
...
async def test_upload_portfolio_success(client):
    ...
    with patch("app.application.use_cases.upload_portfolio.s3_service.upload_file", side_effect=fake_urls):
        response = await client.post(
...
async def test_upload_portfolio_accumulates(client):
    ...
    with patch("app.application.use_cases.upload_portfolio.s3_service.upload_file", return_value=url1):
        await client.post(
            "/api/v1/users/me/portfolio",
            headers={"authorization": f"Bearer {token}"},
            files=[("files", ("first.jpg", BytesIO(b"data"), "image/jpeg"))],
        )
    with patch("app.application.use_cases.upload_portfolio.s3_service.upload_file", return_value=url2):
        response = await client.post(
            "/api/v1/users/me/portfolio",
            headers={"authorization": f"Bearer {token}"},
